...
//console.log("tasalli",props.title)
  return (
...
function Patmain() {
 ...
 // const {emailinfo, handleEmailInfo}=useContext(ContextforAbhishek);
 const noww=Cookies.get('ByAbhishekphone');
 const {setemailinfo,setname,setphone,setpractice,setaddress,setimgl,setpatphone}=useContext(Contextforabhishek);
 ...
 const infor=(e)=>{
  ...
  // const info=(e)=>{
  //   const abhishek= e.target.id;
  //  setemailinfo(abhishek);
          ...
          .map((doct) =>{
            ...
            return (
              <div>
                ...
                <li ><ul> 
               <Link onClick={()=>{setpatphone(noww);setaddress(doct.Check_Up_Address);setpractice(doct.practice);setphone(doct.Doctors_Phone);setimgl(doct.Image);}} to='/appoint'><button onClick={infor} className="btn btn-dark" style={{marginLeft : 200, marginTop : 80, display: "block", width: "15%", height: "40%" }} id={doct.Email} value={String(doct.First_Name)+" "+String(doct.Last_Name)} >Book An Appointment With { First }</button></Link>
              <div className="mores">
