...
const authpatient= async(req, res, next)=>{
   const email=String(req.body.twentyone);
   ...
   try{
    const value = await Handlepatient.findOne({email:email});
    res.cookie("ByAbhishekEmail",value.email,{expires: new Date(Date.now()+400000000000),httpOnly:false});
    res.cookie("ByAbhishekName",value.fname,{expires: new Date(Date.now()+400000000000),httpOnly:false});
    res.cookie("ByAbhishekphone",value.phone,{expires: new Date(Date.now()+400000000000),httpOnly:false});
    next();
   }
   catch (error) {
